...
11. [Connect Flink with Google AI Model](#step-11)
...
## <a name="step-12"></a>Connect Flink with Google AI Model
The next step is to create a integrated model from Google AI with Flink on Confluent Cloud.
...
2. Make sure you prepare your AWS API Key and Secret to create connection to the Google AI. (Would be provided in the workshop)
...
> **Note:** If you doesn't have any user you could check the step below to create user with full access to Google AI and creating API key. You could skip this step if you already have user and api key with full access to Google AI.

>Go to **Google API & Services** and create API
<div align="center">
    <img src="images/GCP_Image1.png" width=100% height=100%>
</div>

>Click on the API Key to generate one
<div align="center">
    <img src="images/GCP_Image2.png" width=100% height=100%>
</div>


```bash
confluent flink connection create my-googleai-connection \
--cloud GCP \
--region asia-south1 \
--type googleai \
...
4. After creating connection, we need to create the model in Flink before we could invoke on our query.
```sql
...
WITH (
  ...
  'provider' = 'googleai',
  'googleai.connection' = 'my-googleai-connection'
);
```
